...
class nnUNetDatasetWithPseudoLabels(nnUNetDatasetBlosc2):
    def __init__(self,
                 original_preprocessed_folder: str,
                 pseudo_preprocessed_folder: str,
                 identifiers: List[str],
                 folder_with_segs_from_previous_stage: str = None):
        
        # Initialize with the ORIGINAL folder to get all properties and GT labels
        super().__init__(original_preprocessed_folder, identifiers, folder_with_segs_from_previous_stage)
        
        # Store the path to the folder with pseudo-labels
        self.pseudo_preprocessed_folder = pseudo_preprocessed_folder

    def __getitem__(self, identifier):
        #identifier = self.get_case_id(index)
        
        # 1. Load the original case (image, GT seg, properties) using the parent method
        data, gt_seg, seg_prev, properties = self.load_case(identifier)

        # 2. Now, load the corresponding pseudo-label from the other folder
        pseudo_seg_b2nd_file = join(self.pseudo_preprocessed_folder, identifier + '_seg.b2nd')
        
        dparams = {'nthreads': 1}
        mmap_kwargs = {} if os.name == "nt" else {'mmap_mode': 'r'}
        pseudo_seg = blosc2.open(urlpath=pseudo_seg_b2nd_file, mode='r', dparams=dparams, **mmap_kwargs)
        
        # 3. Return a dictionary containing everything
        return {
            'data': data,
            'target': gt_seg,          # The original ground truth label
            'pseudo_target': pseudo_seg, # The new pseudo-label
