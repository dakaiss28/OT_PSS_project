...
class nnUNetDataLoaderWithPseudoLabels(nnUNetDataLoader):
    """
    Custom DataLoader that extends the base nnUNetDataLoader to handle an additional pseudo_target.
    """
    def determine_shapes(self):

        data_shape, seg_shape = super().determine_shapes()
        self.pseudo_seg_shape = seg_shape
    ...
    def generate_train_batch(self):
        ...
        data_all = np.zeros(self.data_shape, dtype=np.float32)
        seg_all = np.zeros(self.seg_shape, dtype=np.int16)
        pseudo_seg_all = np.zeros(self.pseudo_seg_shape, dtype=np.int16)

        for j, i in enumerate(selected_keys):
            force_fg = self.get_do_oversample(j)

            loaded_data_dict = self._data[i] 
            data = loaded_data_dict['data']
            seg = loaded_data_dict['target']
            pseudo_seg = loaded_data_dict['pseudo_target']
            properties = loaded_data_dict['properties']
            ...
            # Crop all three data types using the same bounding box
            data_all[j] = crop_and_pad_nd(data, bbox, 0)
            seg_all[j] = crop_and_pad_nd(seg, bbox, -1)
            pseudo_seg_all[j] = crop_and_pad_nd(pseudo_seg, bbox, -1)

        if self.patch_size_was_2d:
            data_all = data_all[:, :, 0]
            seg_all = seg_all[:, :, 0]
            pseudo_seg_all = pseudo_seg_all[:, :, 0]

        batch = {'keys': selected_keys}
        ...
        if self.transforms is not None:
            with torch.no_grad():
                with threadpool_limits(limits=1, user_api=None):
                    data_all = torch.from_numpy(data_all).float()
                    seg_all = torch.from_numpy(seg_all).to(torch.int16)
                    pseudo_seg_all = torch.from_numpy(pseudo_seg_all).to(torch.int16)
                    ...
                    for b in range(self.batch_size):
                        ...
                        # Stack target and pseudo_target into a single multi-channel segmentation
                        stacked_segs = torch.vstack((seg_all[b], pseudo_seg_all[b]))
                        
                        tmp = self.transforms(**{'image': data_all[b], 'segmentation': stacked_segs})
                        images.append(tmp['image'])
                        
                        # Unstack the transformed segmentations
                        segs.append(tmp['segmentation'][0])
                        pseudo_segs.append(tmp['segmentation'][1])
                    ...
                    if isinstance(segs[0], list):
                        # if deepsupervision
                        seg_all = [torch.stack([s[i] for s in segs]) for i in range(len(segs[0]))]
                        pseudo_seg_all = [torch.stack([ps[i] for ps in pseudo_segs]) for i in range(len(pseudo_segs[0]))]
                    else:
                        seg_all = torch.stack(segs)
                        pseudo_seg_all = torch.stack(pseudo_segs)
                        
                    seg_all = seg_all.unsqueeze(1)
                    pseudo_seg_all = pseudo_seg_all.unsqueeze(1)
                    batch['data'] = data_all
                    batch['target'] = seg_all
                    batch['pseudo_target'] = pseudo_seg_all
        else:
            batch['data'] = data_all
            batch['target'] = seg_all
            batch['pseudo_target'] = pseudo_seg_all
