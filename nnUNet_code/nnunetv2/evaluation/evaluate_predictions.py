...
def save_summary_json(results: dict, output_file: str):
    ...
    # convert keys in mean and std metrics
...
def compute_metrics(reference_file: str, prediction_file: str, image_reader_writer: BaseReaderWriter,
    ...
    print(f"Processing: {os.path.basename(reference_file)}")
    # load images
    seg_ref, seg_ref_dict = image_reader_writer.read_seg(reference_file)
    seg_pred, seg_pred_dict = image_reader_writer.read_seg(prediction_file)

    ignore_mask = seg_ref == ignore_label if ignore_label is not None else None
    spacing = seg_ref_dict.get("spacing", None)  # use voxel spacing if available

    labeled_organs = np.unique(seg_ref)
    labeled_organs = labeled_organs[labeled_organs != 0]  # remove background
    ...
    for r in labels_or_regions:
        ...
        if r not in labeled_organs:
            hd95, asd = np.nan, np.nan
        else : 
            
            hd95 = compute_hausdorff_distance(
                    y_pred=np.expand_dims(mask_pred, 0),
                    y=np.expand_dims(mask_ref, 0),
        ...
        results['metrics'][r]['ASD'] = asd


        # make metrics nan for partially labelled sets, so that dice is computed only for available GTs
        if r not in labeled_organs:
            results['metrics'][r]['Dice'] = np.nan
            results['metrics'][r]['IoU'] = np.nan
            results['metrics'][r]['FP'] = np.nan
            results['metrics'][r]['TP'] = np.nan
            results['metrics'][r]['FN'] = np.nan
            results['metrics'][r]['TN'] = np.nan
            results['metrics'][r]['n_pred'] = np.nan
            results['metrics'][r]['n_ref'] = np.nan
...
def compute_metrics_on_folder(folder_ref: str, folder_pred: str, output_file: str,
    ...
    files_ref = sorted([join(folder_ref, i) for i in files_pred])
    files_pred = sorted([join(folder_pred, i) for i in files_pred])
    ...
    """
    results = [] 
    files_to_skip = ['lits_case_0109.nii.gz','lits_case_0113.nii.gz','lits_case_0114.nii.gz','lits_case_0115.nii.gz','lits_case_0116.nii.gz']
    for i, (ref, pred) in enumerate(zip(files_ref, files_pred)):
        ...
        print(f"Processing {i+1}/{len(files_ref)}: {os.path.basename(ref)}")
        try:
            res = compute_metrics(ref, pred, image_reader_writer, regions_or_labels, ignore_label)
            results.append(res)
        except Exception as e:
            print(f"!!! CRITICAL ERROR processing {os.path.basename(ref)}: {e}")
            print("!!! Skipping this file to ensure aggregation continues.")
            continue
    ...
    for m in metric_list:
        ...
        for k in means.keys():
            if k == 0 or k == '0':
                continue
            values.append(means[k][m])
        foreground_std[m] = np.std(values)


    [recursive_fix_for_json_export(i) for i in results]
    recursive_fix_for_json_export(means)
    recursive_fix_for_json_export(foreground_mean)
    result = {'metric_per_case': results, 'mean': means, 'std' : stds, 'foreground_mean': foreground_mean, 'foreground_std' : foreground_std}
    if output_file is not None:
